  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-0.5">Tasks that depend on deliverables from other departments</p>
      </div>
          ...
          return (
            <div key={key} className="bg-white border border-slate-100 rounded-xl overflow-hidden">
              <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full text-white" style={{ backgroundColor: fromDept.color }}>{fromDept.name}</span>
                <ArrowRight className="w-4 h-4 text-slate-300" />
                <span className="text-xs font-semibold px-2.5 py-1 rounded-full text-white" style={{ backgroundColor: toDept.color }}>{toDept.name}</span>
                <span className="ml-auto text-xs text-slate-400">{deps.length} dependenc{deps.length > 1 ? 'ies' : 'y'}</span>
              </div>
              <div className="divide-y divide-slate-50">
                {deps.map(({ task, depTask }) => (
                  <div key={`${task.id}-${depTask.id}`} className="px-5 py-3 grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
                    <button className="text-left hover:bg-slate-50 rounded-lg p-2 transition-colors" onClick={() => setSelected(depTask)}>
                      ...
                      <Badge className={cn('mt-1', statusColor(depTask.status))}>{statusLabel(depTask.status)}</Badge>
                    </button>
                    <div className="flex flex-col items-center gap-1">
                      <ArrowRight className="w-5 h-5 text-slate-300" />
                      <span className="text-[10px] text-slate-300">blocks</span>
                    </div>
                    <button className="text-left hover:bg-slate-50 rounded-lg p-2 transition-colors" onClick={() => setSelected(task)}>
                      ...
                      <Badge className={cn('mt-1', statusColor(task.status))}>{statusLabel(task.status)}</Badge>
                    </button>
                  </div>
                ))}
              </div>
